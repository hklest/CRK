...
public:
  ...
  bool do_SiPM = true; // S13361-2050 series
  string PC;
 ...
 ->//cout << value2 << endl;                                                                                                                                                                                
  ...
  double LMin() // Values chosen to avoid singularities in Sellmeier coefficients. 
  {
    ...
    if(do_CsI)
      {
	lambda_min = 90; // Realistically could be lower, but sellmeier breaks down close to lambda0, which is ~70 nm
      }
    return lambda_min;
  }
 double LMax()
